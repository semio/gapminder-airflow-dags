...
DEFAULT_MAX_AGE_IN_DAYS = Variable.get(
    "max_tmp_age_in_days", 1
...
dag = DAG(DAG_ID, default_args=default_args, schedule=SCHEDULE_INTERVAL, start_date=START_DATE)
...
if [ "${MAX_AGE_IN_DAYS}" == "" ]; then
    echo "maxTmpAgeInDays conf variable isn't included. Using Default '"""
    + str(DEFAULT_MAX_AGE_IN_DAYS)
    + """'."
    MAX_AGE_IN_DAYS='"""
    + str(DEFAULT_MAX_AGE_IN_DAYS)
    + """'
